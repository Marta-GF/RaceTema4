---
title: "Carrera Tidyverse. Preguntas Cortas."
...
```{r}
# En primer lugar, será necesario cargar la librería tibble para usar la función
# que se nos dice en el enunciado
library(tibble)

# Transformamos los datos en un tibble y lo asignamos a cran
cran <- as_tibble(mydf)

# Borramos el data frame original usando la función rm()
rm(mydf)
```
...
```{r}
# Escribimos 'cran' directamente para visualizar los datos
cran

# Observamos 'cran' utilizando la función glimpse
glimpse(cran)
```
...
```{r}
# Para este y los ejercicios siguientes, necesitaremos el uso de la librería
# dplyr, por lo que la cargamos
library(dplyr)

# Seleccionamos únicamente las variables ip_id, package y country mediante una
# tubería. Para escogerlas, usamos la función select()
cranej4 <- cran %>% 
...
```{r}
# Escogemos todas las variables entre r_arch y country usando la función
# del ejercicio anterior pero con : para indicar que es de uno al otro
cranej5 <- cran %>%
...
```{r}
# Elegimos los registros donde se han descargado el paquete plotly desde
# donde se ejecuta la versión de R 3.5.1. Para ello usamos la función
# filter() y aplicamos lo que se nos dice
cran_3_5_1 <- cran %>%
  filter(package == "plotly", r_version == "3.5.1")

# Mostramos los resultados
cran_3_5_1 # (no hace falta head porque solo hay un registro)
```
...
```{r}
# En este caso, usamos la misma metodología del ejercicio anterior, pero
# aplicando los requisitos que se nos dicen
cran_3_4_0 <- cran %>%
...
```{r}
# Escogemos las realizadas en España o Portugal
cran_ES_PT <- cran %>%
...
```{r}
# Para esto, usamos el mismo método que en los ejercicios anteriores
cran_100500_gnu <- cran %>%
  filter(size > 100500, r_os == "linux-gnu")

# Mostramos el resultado
head(cran_100500_gnu)
```
...
```{r}
# En este caso, para escoger los que no tengan la versión del so vacía
# usaremos is.na() para escoger las vacías, y ! para que sean las contrarias,
# es decir, las que no estén vacías
cran_so_no_NA <- cran %>%
  filter(!is.na(r_os))
...
```{r}
# Almacenamos en cran2 todas las columnas entre size y ip_id con el mismo
# método anterior
cran2 <- cran %>%
  select(size:ip_id)
...
```{r}
# Ordenamos cran2 en orden ascendente según la variable ip_id, usando la
# función arrange()
cran2 <- cran2 %>%
  arrange(ip_id) # Es ascendente por defecto
...
```{r}
# Se realiza lo mismo que en el ejercicio anterior, pero especificando 
# que se quiere de manera descendente con desc
cran2 <- cran2 %>%
  arrange(desc(ip_id))
...
```{r}
# Ordenamos cran2 según los valores de package e ip_id con la función arrange
cran2 <- cran2 %>%
...
```{r}
# Ordenamos cran2 como se nos pide, con la misma metodología
cran2 <- cran2 %>%
  arrange(country, desc(r_version), ip_id) # Por defecto es ascendente
...
```{r}
# Hacemos un subconjunto formado por las variables que se nos piden, y lo
# almacenamos en cran3
cran3 <- cran %>%
...
```{r}
# Creamos la nueva variable size_mb con el tamaño expresado en Mbytes, es
# decir, dividiendo size entre 2^20
cran$size_mb <- cran$size/(2**20)
...
```{r}
# Lo mismo que en el anterior pero en Gb (entre 2^10)
cran$size_gb <- cran$size/(2**10)
...
```{r}
# Lo hacemos con el mismo método, sumamos 1000 bytes
cran$correct_size <- cran$size + 1000

#La mostramos
head(cran$correct_size)
```
...
```{r}
# Calculamos el valor medio de lo que se nos pide, y lo almacenamos en avg_bytes
avg_bytes <- cran %>%
  summarise(avg_bytes = mean(size))

# Vemos el valor medio resultante
avg_bytes
```
...
```{r}
# Creamos el agrupamiento del tibble cran según la variable package con la
# función group_by
by_package <- cran %>%
  group_by(package)
...
# Para deshacer el agrupamiento usamos la función ungroup()
...
```{r}
# Calculamos el valor medio de la variable size como se pide
avg_size_by_package <- by_package %>%
  summarise(avg_size_by_package = mean(size))
...
```{r}
# Calculamos las métricas solicitadas en el enunciado
pack_sum <- by_package %>%
  summarise(
    count = n(),                     # Número de elementos
    unique = n_distinct(ip_id),      # Número de elementos distintos de ip_id
    countries = n_distinct(country), # Número de países distintos
    avg_bytes = mean(size),          # Media del tamaño
    med_bytes = stats::median(size)  # Mediana del tamaño
  )

# Vemos el resultado del sumario
pack_sum
```
...
```{r}
# Calculamos las descargas totales para cada paquete
top_counts <- cran %>%
  group_by(package) %>%
  summarise(total_downloads = n()) %>%
  arrange(desc(total_downloads)) %>%
  ungroup() %>%
  filter(row_number() <= n() * 0.01) %>%
  pull(package)

# Vemos los paquetes más descargados
top_counts
```
...
```{r}
# En el ejericicio anterior ya está realizado con pipes (tuberías), por
# lo que sólo habría que ordenarlo en orden descendente
...
# Muestra los paquetes más descargados, ordenados por el número de descargas
top_counts_sorted
```
